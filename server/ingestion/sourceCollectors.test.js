...
test("iCIMS dispatch is registry-owned even when runtime pilot predicate is false", async () => {
  const calls = [];
  const registrySource = {
    atsKey: "icims",
    family: SOURCE_FAMILIES.embeddedOrSemiStructured,
    ...
    discover: () => ({
      ats_key: "icims",
      source_family: "html_detail",
      list_url: "https://fixtureco.icims.com/jobs/search?ss=1"
    }),
    fetchList: async (company, options) => {
      calls.push(["fetchList", company.ATS_name, typeof options.fetcher]);
      return { __legacyParsed: [{ company_name: company.company_name, position_name: "Registry iCIMS Posting" }] };
    },
    parse: (payload) => payload.__legacyParsed,
    normalize: () => null,
    validate: () => ({ ok: true })
  ...
  const runtime = createSourceCollectorRuntime({
    fetchWithAtsRateLimit: async () => {
      throw new Error("iCIMS registry dispatch should not hit legacy network code");
    },
    getPostingLocationByJobUrl: () => new Map(),
    isRegistryPilotSource: () => false,
    getRegistrySourceModule: (atsKey) => {
  ...
  const postings = await runtime.collectPostingsForCompany({
    ATS_name: "icims.com",
    company_name: "iCIMS Registry Co",
    url_string: "https://fixtureco.icims.com/jobs/search?ss=1"
  });

  assert.deepEqual(calls, [
    ["module", "icims"],
    ["fetchList", "icims.com", "function"]
  ]);
  assert.deepEqual(postings, [{
    company_name: "iCIMS Registry Co",
    position_name: "Registry iCIMS Posting"
  }]);
});

test("Loxo dispatch is registry-owned even when runtime pilot predicate is false", async () => {
  const calls = [];
  const registrySource = {
    atsKey: "loxo",
    family: SOURCE_FAMILIES.vendorSpecific,
    ...
    discover: () => ({
      ats_key: "loxo",
      source_family: "html_detail",
      list_url: "https://app.loxo.co/fixtureco"
    }),
    fetchList: async (company, options) => {
      calls.push(["fetchList", company.ATS_name, typeof options.fetcher]);
      return { __legacyParsed: [{ company_name: company.company_name, position_name: "Registry Loxo Posting" }] };
    },
    parse: (payload) => payload.__legacyParsed,
    normalize: () => null,
    validate: () => ({ ok: true })
  ...
  const runtime = createSourceCollectorRuntime({
    fetchWithAtsRateLimit: async () => {
      throw new Error("Loxo registry dispatch should not hit legacy network code");
    },
    getPostingLocationByJobUrl: () => new Map(),
    isRegistryPilotSource: () => false,
    getRegistrySourceModule: (atsKey) => {
  ...
  const postings = await runtime.collectPostingsForCompany({
    ATS_name: "loxo.co",
    company_name: "Loxo Registry Co",
    url_string: "https://app.loxo.co/fixtureco"
  });

  assert.deepEqual(calls, [
    ["module", "loxo"],
    ["fetchList", "loxo.co", "function"]
  ]);
  assert.deepEqual(postings, [{
    company_name: "Loxo Registry Co",
...
test("Talentlyft dispatch is registry-owned even when runtime pilot predicate is false", async () => {
  const calls = [];
  const registrySource = {
    atsKey: "talentlyft",
    family: SOURCE_FAMILIES.vendorSpecific,
    ...
    discover: () => ({
      ats_key: "talentlyft",
      source_family: "html_detail",
      list_url: "https://fixture.talentlyft.com/"
    }),
    fetchList: async (company, options) => {
      calls.push(["fetchList", company.ATS_name, typeof options.fetcher]);
      return { __legacyParsed: [{ company_name: company.company_name, position_name: "Registry Talentlyft Posting" }] };
    },
    parse: (payload) => payload.__legacyParsed,
    normalize: () => null,
    validate: () => ({ ok: true })
  ...
  const runtime = createSourceCollectorRuntime({
    fetchWithAtsRateLimit: async () => {
      throw new Error("Talentlyft registry dispatch should not hit legacy network code");
    },
    getPostingLocationByJobUrl: () => new Map(),
    isRegistryPilotSource: () => false,
    getRegistrySourceModule: (atsKey) => {
  ...
  const postings = await runtime.collectPostingsForCompany({
    ATS_name: "talentlyft.com",
    company_name: "Talentlyft Registry Co",
    url_string: "https://fixture.talentlyft.com/"
  });

  assert.deepEqual(calls, [
    ["module", "talentlyft"],
    ["fetchList", "talentlyft.com", "function"]
  ]);
  assert.deepEqual(postings, [{
    company_name: "Talentlyft Registry Co",
...
test("CalOpps dispatch is registry-owned and stays disabled without hitting legacy dynamic network code", async () => {
  const calls = [];
  const registrySource = {
    atsKey: "calopps",
    family: SOURCE_FAMILIES.publicSectorEducation,
    ...
    discover: () => ({
      ats_key: "calopps",
      source_family: "public_sector",
      list_url: "https://www.calopps.org/job-search-list"
    }),
    fetchList: async () => {
      throw new Error("disabled CalOpps registry source should not fetch");
    },
    parse: (payload) => payload.__legacyParsed || [],
    normalize: () => null,
    validate: () => ({ ok: true })
  ...
  const runtime = createSourceCollectorRuntime({
    fetchWithAtsRateLimit: async () => {
      throw new Error("CalOpps registry dispatch should not hit legacy dynamic network code");
    },
    getPostingLocationByJobUrl: () => new Map(),
    isRegistryPilotSource: () => false,
    getRegistrySourceModule: (atsKey) => {
  ...
  const postings = await runtime.collectPostingsForCompany({
    ATS_name: "www.calopps.org",
    company_name: "CalOpps Registry Co",
    url_string: "https://www.calopps.org/job-search-list"
  });

  assert.deepEqual(calls, [["module", "calopps"]]);
  assert.deepEqual(postings, []);
});
