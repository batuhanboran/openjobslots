...
const OPERATIONAL_SOURCE_METADATA = Object.freeze(require("./pilotSources.json"));
...
const DISABLED_BY_DEFAULT_ATS = new Set(
  Object.entries(OPERATIONAL_SOURCE_METADATA)
    .filter(([, metadata]) => metadata?.status !== "enabled")
    .map(([atsKey]) => atsKey)
);
