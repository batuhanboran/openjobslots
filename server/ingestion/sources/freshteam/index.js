...
const { FRESHTEAM_DOCS_URL, FRESHTEAM_SOURCE_FAMILY, clean, createDiscover } = require("./discover");
const { createFetchList } = require("./fetchList");

const atsKey = "freshteam";
const parserVersion = "source-freshteam-v1";
const parserConfidence = 0.55;
const discover = createDiscover();
const fetchList = createFetchList({ discover });

function normalizeCompanyName(company = {}, fallback = "freshteam") {
...
function parse(rawPayload, company = {}) {
  ...
  return parser.parseFreshteamPostingsFromHtml(
    normalizeCompanyName(company, config.subdomainLower || "freshteam"),
    config,
    html
  );
}
...
function normalize(posting, company = {}, options = {}) {
  ...
  normalized.source_family = FRESHTEAM_SOURCE_FAMILY;
  normalized.evidence = buildEvidenceMetadata(normalized, {
    parserVersion,
    sourceFamily: FRESHTEAM_SOURCE_FAMILY
  });
...
function rateLimit() {
  return {
    requestsPerMinute: 8,
    strategy: "freshteam-jobs-page-per-host-serialized"
  };
}
...
function fixtures() {
  return [
    "server/ingestion/sources/freshteam/fixtures/list.json",
    "server/ingestion/sources/freshteam/fixtures/expected-normalized.json",
    "server/ingestion/sources/freshteam/fixtures/invalid-shapes.json"
  ];
}
...
module.exports = {
  ...
  status: "disabled",
  parserVersion,
  officialDocs: FRESHTEAM_DOCS_URL,
  discover,
  fetchList,
