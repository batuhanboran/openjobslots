...
function assertApplicantAiFinalHost(finalUrl, fallbackUrl) {
  const value = clean(finalUrl || fallbackUrl);
  try {
    const host = new URL(value).hostname.toLowerCase();
    if (isSupportedApplicantAiHost(host)) return;
  } catch {
    // Fall through to the source error below.
  }
  throw makeSourceFetchError("unexpected_redirect_host", `ApplicantAI URL redirected to unexpected host: ${value}`, {
    url: value
  });
}
...
function createFetchList(dependencies = {}) {
  ...
  return async function fetchApplicantAiSourceList(company = {}, options = {}) {
    ...
    if (!careersUrl) {
      throw makeSourceFetchError("no_public_jobs_route", "ApplicantAI source has no supported public careers route");
    }

    const target = {
      method: "GET",
      headers: buildHeaders(),
      source_key: "applicantai",
      source_family: APPLICANTAI_SOURCE_FAMILY,
      rateLimitMs: APPLICANTAI_RATE_LIMIT_WAIT_MS
    };
    ...
    if (typeof options.fetcher === "function") {
      ...
      if (status < 200 || status >= 300) {
        throw makeSourceFetchError("fetch_failed", `ApplicantAI page request failed (${status})`, {
          status,
          url: careersUrl
        });
      }
      const finalUrl = clean(payload?.url || payload?.__sourceFetchFinalUrl || careersUrl);
      assertApplicantAiFinalHost(finalUrl, careersUrl);
      return {
    ...
    if (!res.ok) {
      ...
      throw makeSourceFetchError(
        "fetch_failed",
        `ApplicantAI page request failed (${res.status}): ${body.slice(0, 180)}`,
...
module.exports = {
  APPLICANTAI_RATE_LIMIT_WAIT_MS,
  createFetchList
};
