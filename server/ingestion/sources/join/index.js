...
const atsKey = "join";
const parserVersion = "source-join-v1";
const parserConfidence = 0.55;
const discover = createDiscover();
const fetchList = createFetchList({ discover });

function normalizeCompanyName(company = {}, fallback = "join") {
...
function parse(rawPayload, company = {}) {
  ...
  const nextData = payload?.html ? parser.extractJoinNextDataJsonFromHtml(payload.html) : payload;
  return parser.parseJoinPostingsFromNextData(
    normalizeCompanyName(company, config.companySlugLower || "join"),
    config.companySlug || config.companySlugLower || "",
    nextData
  );
}
...
function normalize(posting, company = {}, options = {}) {
  ...
  normalized.source_family = JOIN_SOURCE_FAMILY;
  normalized.evidence = buildEvidenceMetadata(normalized, {
    parserVersion,
    sourceFamily: JOIN_SOURCE_FAMILY
  });
...
function rateLimit() {
  return {
    requestsPerMinute: 1,
    strategy: "join-company-page-per-host-serialized"
  };
}
...
function fixtures() {
  return [
    "server/ingestion/sources/join/fixtures/list.json",
    "server/ingestion/sources/join/fixtures/expected-normalized.json",
    "server/ingestion/sources/join/fixtures/invalid-shapes.json"
  ];
}
