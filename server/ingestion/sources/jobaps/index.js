...
const ATS_KEY = "jobaps";
const SOURCE_FAMILY = "public_sector";
const PARSER_VERSION = "source-jobaps-v1";
const PARSER_CONFIDENCE = 0.55;

const discover = createDiscover();
const fetchList = createFetchList({ discover });

function normalizeCompanyName(company = {}, fallback = "jobaps") {
...
function parse(rawPayload, company = {}) {
  ...
  const companyName = normalizeCompanyName(company, config.tenantTokenLower || "jobaps");
  const listUrl = clean(rawPayload?.__sourceFetchFinalUrl || config.boardUrl || target.list_url);
  const postings = parser.parseJobApsPostingsFromHtml(companyName, config, html, listUrl);

  return postings.map((posting) => ({
    ...posting,
    source_job_id: clean(posting.source_job_id || posting.external_id),
    source_evidence: {
      ...(posting.source_evidence || {}),
      list_url: listUrl,
      route_kind: posting.source_evidence?.route_kind || "jobaps_public_list"
    }
  }));
}
...
function fixtures() {
  return [
    "server/ingestion/sources/jobaps/fixtures/list.json",
    "server/ingestion/sources/jobaps/fixtures/expected-normalized.json",
    "server/ingestion/sources/jobaps/fixtures/invalid-shapes.json"
  ];
}
