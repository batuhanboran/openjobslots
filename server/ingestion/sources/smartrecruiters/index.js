...
const baseModule = createSourceModule("smartrecruiters");
...
function parse(rawPayload, company = {}) {
  ...
  const payload = rawPayload && typeof rawPayload === "object" && !Array.isArray(rawPayload)
    ...
    : rawPayload;
  return parser.parseSmartRecruitersPostingsFromApi(
    clean(company.company_name || company.companyName || company.name || config.companySlug || "smartrecruiters"),
    config,
    payload
  );
}
