...
const PARSER_VERSION = "source-getro-v1";
const PARSER_CONFIDENCE = 0.75;

const discover = createDiscover();
const fetchList = createFetchList({ discover });

function normalizeCompanyName(company = {}, fallback = "getro") {
...
function parse(rawPayload, company = {}) {
  ...
  const companyName = normalizeCompanyName(company, config.subdomainLower || "getro");
  const listUrl = clean(rawPayload?.__sourceFetchFinalUrl || config.jobsUrl || target.list_url);
  const postings = parser.parseGetroPostingsFromHtml(companyName, config, html);
  ...
  return postings.map((posting) => ({
    ...
    source_evidence: {
      ...
      list_url: listUrl,
      route_kind: posting.source_evidence?.route_kind || "getro_next_data_jobs"
    }
  }));
}
...
function fixtures() {
  return [
    "server/ingestion/sources/getro/fixtures/list.json",
    "server/ingestion/sources/getro/fixtures/expected-normalized.json",
    "server/ingestion/sources/getro/fixtures/invalid-shapes.json"
  ];
}
