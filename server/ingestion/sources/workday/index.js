...
const baseModule = createSourceModule("workday");
const discover = createDiscover();
const fetchList = createFetchList({ discover });

function normalizeCompanyName(company = {}, fallback = "workday") {
...
function parse(rawPayload, company = {}) {
  ...
  const companyName = normalizeCompanyName(company, config.site || config.subdomain || "workday");
  return parser.parseWorkdayPostingsFromApi(companyName, config, stripInternalPayloadFields(rawPayload));
}
