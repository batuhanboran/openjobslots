...
- A canary/apply guard that requires explicit production flags, backup confirmation, worker isolation proof, a fresh passing preflight report with `generated_at` and non-empty backup-file proof, a readable source-matched `ats:plan-batches` report with a selected tenant plan and `predicted_guard_result=pass`, scopes operations to selected tenant targets only, requires `--max-updates=N` for apply writes, and records a rollback path.
- A release report that records inventory proof, timestamped preflight proof with non-empty backup file and safe runtime/search state, net-new clean public estimate, duplicate public candidates excluded from gain, candidate-pool proof, estimate confidence, the tenant batch plan, predicted guard result, audited rollback command, bounded Meili outbox/upsert status, and full `search:reindex:check` parity: count, remote facets, sampled fields, document drift, and settings.
