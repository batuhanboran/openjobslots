...
- A canary/apply guard that requires explicit production flags, a readable source-matched `ats:plan-batches` report with a selected tenant plan and `predicted_guard_result=pass`, and a rollback path.
