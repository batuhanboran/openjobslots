...
Before calling an ATS recovery release successful, the source recovery report and release inputs must also carry the inventory, preflight, batch-plan, rollback, and search-sync proof consumed by `release:ats-recovery:check`: `inventory_scan_report`, `net_new_clean_public_estimate`, `duplicate_existing_public_candidates`, `candidate_pool_exhausted`, `estimate_confidence`, `planned_tenant_batch_file_path` or `planned_batch_report`, `predicted_guard_result`, `rollback_command`, `bounded_outbox_or_upsert_status`, a passing preflight report with `generated_at`, non-empty backup-file proof, production/expected commit proof, safe long-query/heavy-job/worker/autodeploy state, and a full `search:reindex:check` parity report. The search parity proof must have `ok=true`, `count_delta=0`, empty `remote_facet_delta`, no sampled field/missing-document drift, no extra/missing Meili documents, and no settings mismatches. Missing or unproven fields are release blockers, even when parser fixtures and data-quality percentages pass.

The source-run path enforces the same batch-plan and preflight boundary. `ats:source:canary` / `ats:source:apply` operation authorization requires `--planned-batch=<report>`, `--preflight-report=<report>`, and `--predicted-guard-result=pass` alongside `--confirm-production`, `--backup-confirmed`, `--worker-isolated`, source recovery readiness, and the later recovery guard; apply writes additionally require `--max-updates=N`. `--worker-paused` is accepted as an alias for the worker isolation proof. The planned-batch file must be readable JSON from `ats:plan-batches`, marked `read_only`, source-matched to the requested ATS, and include a selected plan with selected tenants, positive selected gain, zero selected no-geo/no-remote candidates, and `predicted_guard_result=pass`. The preflight report must be safe (`ok=true`, `unsafe=false`), include no failures, carry a parseable `generated_at` within the source-run freshness window, prove expected/actual production commit, worker/autodeploy/heavy-job/search/long-query state is safe, and prove a real backup file exists under `backups/` with `backup_size_bytes>0`. After both reports are accepted, source-run canary/apply operations are scoped to the selected tenant targets from the plan; out-of-plan discovered targets are skipped before fetch/write work, and a command that discovers no selected target is blocked before creating a source run. A canary/apply command without a valid passing planned batch and operational safety proof remains unauthorized even when other production flags are present.
