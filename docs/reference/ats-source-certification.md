...
| --- | --- | --- | --- |
| `greenhouse` | Greenhouse Job Board API JSON. | The source has no universal remote flag, and office labels can mix geo and work-mode evidence. | v1.6 adds raw `jobs[]` fixture with location, offices, department, content, source id, and tracking-query canonicalization. Wave B prefers `first_published` over `updated_at`, preserves city/location evidence, derives office country for remote/hybrid list labels, rewrites safe `Country - City` values, and limits office-level `Remote` evidence to country-only or generic multi-location labels. Add retrieve-job fixture only if detail-only fields become indexed. |
...
| `saphrcloud` | SAP SuccessFactors API/HTML parser. | Locale/date/location brittle; live tenant variants still need canary evidence. | Source-owned board HTML fetch plus API fixture parser carry stable source ids; add broader pagination/API fallback fixtures before promotion. |
...
| `jobvite` | Jobvite careers HTML tables plus per-job JSON-LD detail pages. | List HTML still omits date and can expose numeric multi-location labels; detail pages may omit structured `jobLocation` for some rows. | Source module now fetches bounded detail pages, carries `datePosted`, structured city/country/region, employment type, URL source id, and list Remote/Hybrid prefixes. Numeric multi-location labels are quarantined unless detail JSON-LD supplies geo. Keep canary-first until inventory/net-new estimate proves clean public row gain. |
...
| `zoho` | Zoho hidden `jobs` JSON in careers page. | Hidden job id, city/state/country, date opened, and industry are certified when present; missing id rows are skipped and missing-title rows are rejected. | v1.5.24 adds saved raw hidden-input fixture and failure fixture. Wave B emits explicit city/state/country, guards hidden list URLs so off-host metadata cannot redirect parsing, and relies on shared aliases for real country payload tokens observed live such as Ghana, Costa Rica, Sri Lanka, Suriname, Papua New Guinea, and Cote d'Ivoire. Next: add malformed JSON and unpublished-job variants, and keep remote/no-location rows countryless unless source payload exposes a country. |
| `breezy` | Breezy portal HTML cards and public JSON payloads. | Relative dates may not parse when source does not provide absolute dates; `Worldwide` remote rows remain countryless by design. | v1.5.24 adds saved raw card fixture and failure fixture for missing title/URL; `/p/{id}` source ids, location, date, department, and hybrid evidence are certified when present. Public JSON country payload aliases now cover `Bermuda`, `Virgin Islands, British`, `Togo`, `Gabon`, `Cameroun`, and `中国`. Add detail-page fixtures for fields omitted from list cards. |
| `applicantpro` | Board HTML discovers domain id, then core jobs JSON. | Domain discovery is still brittle, but the JSON jobs parser now preserves `id`, `city`/`iso3`, `startDateRef`, source `workplaceType`, department, and employment type when source exposes them. ApplicantPro source-local normalization maps observed `iso3` country evidence and clears country-only labels from `city`. | v1.5.21 adds a pure `parseApplicantProPostingsFromApi` parser plus raw API fixture. Next: add board HTML domain-id fixture, `success=false` failure fixture, and live sampled location variants. |
...
| `hrmdirect` | HRMDirect table rows. | Non-US city/province pairs still need broader country evidence. | Wave B adds raw table fixtures, expected normalized fixtures, and failure fixtures. Parser now carries `req` source id, absolute URL host guard, city/state-derived location, country when state/location proves it, Puerto Rico city plus three-digit numeric `td.state` list evidence, department, date, job type/employment type, and remote-city guard. Add more tenant-specific sparse-cell variants before broad backfill. |
| `isolvisolvedhire` | Board HTML `domain_id` bootstrap then core jobs JSON. | API rows may omit `workplaceType`, leaving remote unknown by source evidence. | Source-local fixtures now preserve `id`, canonical `jobUrl`, `startDateRef`, `iso3`, structured city/state fields, and `workplaceType`; monitor live canary quality and add more sparse variants before broader promotion. |
...
| `join` | JOIN Next.js embedded data. | Source-local fixtures preserve `idParam` source ids, structured `city.cityName`/`city.countryName`, and remote/workplace type. Observed `countryName` tokens such as Bangladesh, Kosovo, Ghana, Costa Rica, Albania, Liechtenstein, Uganda, Venezuela, Reunion, and Bosnia and Herzegovina are mapped inside the JOIN module. | Add pagination, empty-board, and additional country variants before public promotion. |
...
| `talexio` | Talexio jobs JSON. | Live variants still need canary evidence before promotion. | Source-owned API fetch now carries vacancy `id` as `source_job_id`; fixtures cover country normalization, remote/onsite, invalid shapes, and collector dispatch. |
