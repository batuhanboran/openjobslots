...
- Embedded/HTML: `icims` and `applitrack` are certified but need detail-refetch-backed field repair for live data gaps. `isolvisolvedhire` and `theapplicantmanager` now have source-local raw fixtures; `theapplicantmanager` remains disabled until live canary evidence proves quality. `jobvite`, `talentreef`, and `hirebridge` remain raw-fixture blockers.
