...
| `icims` | iCIMS wrapper page, iframe, paged cards, and public job detail pages. | Public posted dates are frequently absent; blank location cards still require sampled detail fetch; pagination selectors remain brittle. | v1.5.19 adds saved list/detail raw fixtures, `US-PA-Philadelphia`/`CA-ON-Toronto` country-code location parsing, `US-Remote` country recovery, and explicit `Remote: Yes/No` handling. Continue detail probes for blank-location tenants and next-page variants. |
