...
- Strict saved raw parser-fixture-backed ATS keys: 6 (`adp_workforcenow`, `fountain`, `oracle`, `paylocity`, `pinpointhq`, `recruitcrm`).
- Inline direct parser regression coverage also exists for `applytojob`, `bamboohr`, `breezy`, `recruitee`, `taleo`, and `workday`, but those are not counted as strict saved raw fixture certification until their source responses are saved as fixtures. `icims` and `applitrack` now have saved raw detail certification fixtures for their P0 field-recovery paths.
...
| --- | --- | --- | --- |
| `greenhouse` | Greenhouse Job Board API JSON. | Remote inferred from text; raw direct fixture still pending. | v1.5.16 carries source id and department when exposed; add raw `jobs[]` parser fixture with nested location and remote text cases. |
| `lever` | Lever postings API JSON. | Broad regional remote can miss country; raw direct fixture still pending. | v1.5.16 carries source id, team, and commitment; add raw fixture for categories, multiple locations, hosted URL fallback. |
...
| `smartrecruiters` | Public SmartRecruiters search JSON. | Rows can drop when `applyUrl` is absent; industry still needs broader fixture coverage. | v1.5.16 carries source id from JSON/URL; add `content[]` fixture and pagination test. |
...
| `icims` | iCIMS wrapper page, iframe, paged cards, and public job detail pages. | Public posted dates are frequently absent; blank location cards still require sampled detail fetch; pagination selectors remain brittle. | v1.5.19 adds saved list/detail raw fixtures, `US-PA-Philadelphia`/`CA-ON-Toronto` country-code location parsing, `US-Remote` country recovery, and explicit `Remote: Yes/No` handling. v1.5.20 makes the production backfill replace conflicting old country values when `CC-state-city` evidence is stronger. Continue detail probes for blank-location tenants and next-page variants. |
| `zoho` | Zoho hidden `jobs` JSON in careers page. | Strong fields; raw hidden-input fixture is still pending. | v1.5.16 maps job id to source id; add malformed JSON and location/date variant tests. |
...
| `manatal` | Careers-page runtime config plus jobs API/fallback HTML. | Apply URL and config extraction remain brittle in some boards. | v1.5.16 carries source id from `/job/{id}` or API id/hash; add API pagination plus fallback HTML fixtures. |
...
| `applitrack` | Applitrack `Output.asp?all=1` plus `JobPostings/view.asp` detail pages. | Existing rows are often blank because `Output.asp` omits geo/date; full repair requires polite detail-page backfill, not only normalization. | v1.5.19 adds saved `Output.asp` plus detail fixtures, certifies `JobPostings/view.asp?AppliTrackJobId=...` detail URLs, and parses detail date/location/remote evidence. Run bounded `backfill:detail-pages` before reindexing Applitrack-heavy repair batches. |
