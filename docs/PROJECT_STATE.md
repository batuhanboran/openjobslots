...
- Collector extraction is now the active architecture lane: HRMDirect, BambooHR, ApplyToJob, Breezy, CareerPlug, CareerPuck, GovernmentJobs, SchoolSpring, ApplicantPro, Ashby, Lever, SmartRecruiters, Zoho, RecruitCRM, Recruitee, Fountain, Applitrack, Taleo, PinpointHQ, Join, Rippling, Manatal, Teamtailor, Freshteam, Getro, UltiPro, Workday, ADP MyJobs, ADP WorkForceNow, Greenhouse, iCIMS, isolvisolvedhire, Jobvite, Gem, Paylocity, BrassRing, HireBridge, PageUp, Eightfold, TalentReef, TalentLyft, Oracle, Loxo, CareersPage, and JobAps route through `server/ingestion/sourceRegistry.js` and their own `server/ingestion/sources/<ats>/` modules. `server/ingestion/sourceCollectors.js` should continue shrinking one ATS family at a time.
- Current collector target chain: SchoolSpring is the latest local architecture extraction checkpoint and remains `enabled` in the registry to preserve its previous public-enabled dynamic-collector posture while enabling guarded estimates/certification through the source module. SchoolSpring-specific public JSON API discovery, bounded page fetch, date-window filtering, final-host guards, request metadata, source-id preservation from `value.jobsList[].jobId`, public-source fixtures, and invalid-shape behavior now live under `server/ingestion/sources/schoolspring/`; `server/ingestion/sourceCollectors.js` no longer owns SchoolSpring-specific dynamic collector logic. Raman's fallback explorer ranked Simplicant next and K12JobSpot second after SchoolSpring. No source apply, backfill, cleanup, worker budget change, deploy, or Meili reindex is implied by this target.
