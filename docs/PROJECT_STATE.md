...
- The follow-up revision uses a narrow Postgres freshness update for unchanged visible postings and a Meilisearch `PUT` partial update containing only `id`, `canonical_url`, and `last_seen_epoch`; full documents remain mandatory for actual search-projection changes. Unchanged rows advance freshness at most once per `6h` checkpoint instead of every crawler pass, while real search-content changes remain immediate. It also defers expensive maintenance on worker startup and changes retention, source-quality protection, and public-stat snapshot defaults to every `6h`. Search-outbox draining still runs after every worker run.
- Startup reconciliation now treats the retired `adpmyjobs` source marker as durable completion evidence and skips the already-finished alias migration. Before this guard, every app/worker restart repeated no-op joins across `posting_cache` and `postings`; production `pg_stat_statements` attributed the restart-only multi-gigabyte read burst to those two statements.
- The production Meilisearch version is `v1.16`; it rejects the newer `skipCreation` document parameter. Freshness writes therefore use the v1.16-compatible partial `PUT /documents` endpoint. The rejected parameter had kept outbox rows pending and replayed the same full-document task on every run; the compatible endpoint allows the durable outbox to drain.
...
- Production was read-only checked on the production host at `<app-dir>` and still runs older code at SHA `72eac3f`; `/health` reports `configured_ats_count=60`, `job_slot_count=347,129`, `visible_ats_count=37`, and `postings_seen_24h_count=35,766`, so the local Personio/Workable modules and Manatal/Gem hardening are not live yet.
- Target source blockers are still production-state blockers, not local parser-contract blockers: `workday` and `manatal` are auto-disabled for stale `http_blocked` source-quality reasons, `gem` and `adp_workforcenow` are auto-disabled for parser drift, `dayforcehcm` remains excluded from default sync pending bounded direct-fetch proof, production has no `personio` or `workable` source rows yet, and production has legacy ADP alias state that must be canonicalized into `adp_workforcenow`.
- `scripts/report-ats-target-table.js` now canonicalizes legacy ATS aliases in read-only reports, so stale DB rows such as `adpworkforcenow` are folded into `adp_workforcenow` instead of appearing as separate ATS families. The checked-in target-table snapshot is still a stale production snapshot until it can be regenerated with Postgres credentials.
- `scripts/report-disabled-ats-recovery-plan.js` now generates a read-only disabled-source recovery plan by combining local source-module/recovery-contract readiness with production `ats_sources`, source-quality, cache, visible-row, and latest source-run state. The current generated report is `docs/reference/ats-disabled-recovery-plan.json` / `.md`: all seven target sources are locally ready at the recovery-contract layer, but all seven remain production-gated by source protection, missing production rows, default-sync canary scope, or legacy ADP alias rows.
- `scripts/plan-ats-source-state-repair.js` now turns that read-only recovery plan into an approval-gated source-state repair plan at `docs/reference/ats-source-state-repair-plan.json` / `.md`. The current plan hash is `fe452eadb3a03192`; it has `2` source rows to seed (`personio`, `workable`), `4` source protections to reset to canary (`workday`, `manatal`, `gem`, `adp_workforcenow`), and `1` legacy alias canonicalization (`adpworkforcenow` -> `adp_workforcenow`). Dayforce/Personio/Workable next-step canary commands include `--include-disabled` so explicit source-run proof can target disabled-by-default canary rows.
- `scripts/apply-ats-source-state-repair.js` now provides the approval-gated executor for that plan. Default mode is dry-run; production execution requires `--execute`, `--confirm-production`, `--expected-plan-hash=<hash>`, a fresh safe `--preflight-report=<report>`, backup proof, worker isolation, Meili/Postgres delta `0`, and explicit ADP alias flags (`--allow-alias-canonicalization --alias-conflict-reviewed`) before alias writes. The current dry-run artifact is `docs/reference/ats-source-state-repair-apply-dry-run.json` and selects `7` source-state actions without touching production.
- `scripts/report-ats-alias-conflicts.js` now checks ADP legacy alias canonicalization before any alias write. Fresh read-only production evidence is saved at `docs/reference/ats-alias-conflicts-prod-readonly.json`: `adpworkforcenow` has `416` legacy rows total and one supported `company_sync_state` unique-key conflict for `Circular Action Alliance`; straight alias UPDATE is not safe. The executor can merge that supported sync-state conflict only when the operator also supplies `--allow-alias-conflict-merge` plus a matching alias-conflict report. Any unsupported conflict table remains blocked.
- `package.json` now groups the disabled recovery plan, source-state repair plan, approval-gated apply, and alias-conflict tests behind `npm run test:ats-source-state-repair`, and `npm run test:backend` invokes that composite guard so future backend gates cover this production-state repair path.
- `scripts/reindex-meili-from-postgres.js` now has an approval-gated bounded document-upsert repair path for the current index threshold blocker: `npm run reindex:meili -- --repair-document-upserts --document-repair-limit=100 --json` plans missing-document and stale remote-facet upserts without writing on the current local code, and apply requires `--apply --confirm-production --preflight-report=<fresh_preflight_report>`. Production still runs older SHA `72eac3f`, where those new flags are not supported; do not run that command on production again until the local script is deployed and the dry-run/apply gates are verified on the deployed code. The preflight can tolerate the current nonzero Meili/Postgres delta only when that is the sole unsafe finding, because this repair targets that drift; backup proof, worker isolation, autodeploy safety, no active heavy-job lock, no unsafe long-running queries, and commit proof are still required. The remote-facet inspector now falls back from search hits to filtered Meili `/documents` scans when `maxTotalHits` hides stale facet rows. The older read-only plan at `docs/reference/meili-document-upsert-repair-plan-prod-readonly.json` is stale after the production Meili incident below and must be regenerated after deploy.
- The local Meili reindex CLI now rejects unknown arguments before opening any reindex path. This is a deploy-time safety guard for the production incident class where a misspelled or unsupported repair flag could otherwise be silently ignored and fall through to an unintended incremental write path.
- Fresh read-only production checks still block threshold success: `search:reindex:check -- --json --sample-limit=25` inside `openjobslots-app` reports `ok=false`, Postgres indexable `347,123`, Meili docs `347,117`, count delta `6`, missing Meili documents `6`, and remote facet drift (`onsite +57`, `remote +8`, `hybrid +1`, `unknown -60`). `audit:source-freshness -- --hours=24 --json` remains `decision=hold` with target success `91.74%`, rows seen `35,766`, rows new `5,268`, `0` new public no-geo/no-remote rows, but unresolved parser attention `1`, current-policy parser-bug failures `2`, source-quality failures `236`, empty-no-jobs `96`, auth `33`, and network `5`.
- Production write incident: `npm run reindex:meili -- --repair-document-upserts --document-repair-limit=100 --json` was run against production SHA `72eac3f` expecting local dry-run behavior, but the old script ignored the new flags and began active Meili upserts before failing with an `ECONNRESET`. This wrote only to the derived Meili index, not Postgres/source-state rows. Follow-up checks show app/worker/Postgres/Meili running, `/health` OK, public posting searches responding, Meili restart count `4`, and the threshold still blocked by the same `6` missing documents. No further production write command should be run without explicit approval, deployed current code, backup proof, worker isolation, and a fresh preflight.
- No production deploy, source canary/apply, data backfill, source protection write, backup, or worker isolation was run for this local architecture update.
...
- `server/ingestion/sourceRunner.js` now refuses source-run canary/apply operation authorization unless the request includes `--confirm-production`, `--backup-confirmed`, `--worker-isolated`, a passing recovery-readiness gate, a valid source-matched `--planned-batch=<report>`, a fresh `--preflight-report=<report>`, and `--predicted-guard-result=pass`. The preflight report must include a parseable `generated_at` within the freshness window, defaulting to `60` minutes and configurable with `--preflight-max-age-minutes=N`. Apply writes additionally require `--max-updates=N`; `--worker-paused` is accepted as an alias for worker isolation.
- `scripts/release-ats-recovery-check.js` now revalidates the preflight proof before release success claims: `ok=true` alone is insufficient if `generated_at`, backup file proof, production/expected commit, long-query state, Meili/Postgres delta, worker/autodeploy state, or heavy-job state is missing or unsafe.
- The same release check now treats `search:reindex:check` as full parity proof, not count-only proof: `ok=true`, zero count delta, empty remote facet delta, no sampled field/missing-document drift, no extra/missing Meili documents, and no settings mismatches are required.
...
- Verification covered `node --check server\ingestion\sourceRunner.js`, `node server\ingestion\sourceRunner.test.js`, `node --check scripts\release-ats-recovery-check.js`, `node scripts\release-ats-recovery-check.test.js`, `npm.cmd run test:backend`, `npm.cmd run audit:architecture-boundary -- --json`, `npm.cmd run ats:registry-index -- --json --no-write`, `npm.cmd run release:ats-recovery:check -- --self-test --json`, and `git diff --check`. No production source apply, canary/apply, data backfill, public-row delete/hide, Meili repair/reindex, deploy, backup, cleanup, or worker-budget change was run.

## ATS Architecture & Recovery v2 Baseline - June 1, 2026

- Read-only production baseline was refreshed from the production host; production `<app-dir>` is at `6660eab`, while local `main` is ahead with source-local parser ownership and v2 architecture guard commits.
...
- Local architecture checkpoint after commit `836d6c2`: production is still `<app-dir>` `6660eab`, public rows were freshly observed at `332,539`, and source freshness stayed on `hold` with `160` unresolved parser-attention events. Existing parser fixes remain local until an explicit deploy/source refresh.
- `server/ingestion/sources/index.js` now builds startup source-module inventory from every source-local directory with an `index.js`. This removes the transitional `LOCAL_ONLY_SOURCE_ATS_KEYS` exception and makes the local source contract modules visible through `DIRECT_SOURCE_ATS_KEYS` and `sourceModules`, not just the legacy `SOURCE_SPECS`/exception modules.
- Local registry/workbench state now includes `personio` and `workable` as canary, fixture-backed source modules. `ats:registry-index -- --json --no-write` reports `configured_ats_count: 62`, `read_only_recovery_ready_count: 62`, and no recovery readiness blockers, but production still reports the older configured ATS count until these local changes are deployed and seeded.
...
- Fresh read-only production check still has `<app-dir>` at `6660eab`; app, worker, Postgres, and Meili are running, `/health` is `ok`, and public visible/job-slot count is `332,629`. Local recovery commits remain undeployed until explicit approval.
...
- Verification covered `node --check server\ingestion\sourceRunner.js` and `node server\ingestion\sourceRunner.test.js` with negative tests for missing batch proof, unreadable batch report, source mismatch, failed report prediction, failed CLI prediction, shared-host target URL scoping, out-of-plan target skipping, and no matching selected target. No production source apply, canary/apply, data backfill, public-row delete/hide, Meili repair/reindex, deploy, backup, cleanup, or worker-budget change was run.

## ATS Recovery v2 Edge-Shape Hardening - June 1, 2026

- Local `main` now includes the ATS recovery proof-gate and edge-shape commits `401720b`, `5392e04`, `5613703`, `f98d75d`, `ed501f4`, and `9a6da08`. These are local-only until deploy/source refresh; production `<app-dir>` was last verified separately at `6660eab` during the refreshed baseline.
...
- Fresh read-only production evidence kept production at `<app-dir>` `6660eab` with `332,387` visible job slots and source freshness still on `hold`. Breezy was the top current parser-attention source with `88` recent parser-bug events and `4,045` quarantined cache rows, mostly `no_geo_no_remote` or ambiguous multi-location labels.
...
- Fresh read-only production checks kept `<app-dir>` at `6660eab`; public health reported `331,698` visible job slots and Meili/Postgres count parity with the known `40` document remote-facet drift still unresolved. No Meili repair or replace reindex was run.
...
- Fresh production health during this lane kept `<app-dir>` at `6660eab` with app/worker/Postgres/Meili running and public health at `331,640` visible job slots. `search:reindex:check -- --json --sample-limit=25` had Postgres/Meili count parity (`331,653`/`331,653`) but remained `ok=false` because remote facets drifted by `40` onsite vs unknown documents. No Meili repair or replace reindex was run.
...
- Fresh read-only production checks kept `<app-dir>` at `6660eab`; public health reported `331,524` visible job slots and Meili/Postgres document counts remained aligned with the known `22` document remote-facet drift unresolved.
...
- Fresh read-only production checks kept `<app-dir>` at `6660eab`; public health reported `331,515` visible job slots. Meili/Postgres document counts were still aligned at `331,509` each, with the known `22` document remote-facet drift still unresolved. No Meili repair or replace reindex was run.
...
- Fresh read-only production checks kept `<app-dir>` at `6660eab`, all four services running, public health at `331,524` visible job slots, and `search:reindex:check -- --json --sample-limit=25` at Postgres/Meili count parity (`331,518`/`331,518`) with the known `22` document remote-facet drift. No Meili repair or replace reindex was run.
...
- Fresh read-only production checks kept `<app-dir>` at `6660eab` with app/worker/Postgres/Meili running. Public health reported `331,490` visible job slots. `search:reindex:check -- --json --sample-limit=25` still had Postgres/Meili count parity (`331,484`/`331,484`) but remained `ok=false` because remote facets drift by `22` onsite vs unknown documents. No Meili repair or replace reindex was run.
...
- Fresh <PROD_HOST> checks kept production at `6660eab` with all four runtime services running. Public health still reports `331,463` visible job slots. `search:reindex:check -- --json` still has Postgres/Meili count parity (`331,457`/`331,457`) but remains `ok=false` because remote facets drift by `6` onsite vs unknown.
...
- Read-only reports:
  - Inventory: `<app-dir>/reports/zoho-v2-readonly-inventory-small-20260531T221402Z.json`.
  - Net-new estimate: `<app-dir>/reports/zoho-v2-readonly-estimate-small-20260531T221544Z.json`.
  - Batch plan: `<app-dir>/reports/zoho-v2-readonly-plan-small-20260531T221901Z.json`.
...
- Production canary enablement opened the disabled-normal ATS batch as `canary_only` after a fresh Postgres backup at `<app-dir>/backups/v2-ats-canary-enable-20260527T134552Z.dump`. At that time Dayforce stayed disabled/unsupported; the June 1 DayforceHCM parser certification above supersedes that classification while still keeping Dayforce disabled. Auto-disabled `manatal`, `pinpointhq`, `taleo`, and `workday` remain blocked by their recorded failure reasons.
...
- Production deployment posture after this architecture update is `main` on <PROD_HOST> `<app-dir>`; app and worker should be rebuilt/recreated when parser modules change.
...
- Production host/path: the deploy host, `<app-dir>`.
...
- Production host: the deploy host.
...
Reports were written on <PROD_HOST> under `<app-dir>/reports/`.

- Certified rebuild backup: `<app-dir>/backups/postgres-openjobslots-pre-certified-rebuild-20260512-155252.dump`.
- Final data-quality audit JSON: `<app-dir>/reports/v180-final-20260512-175855-data-quality.json`.
- Final ATS quality audit JSON: `<app-dir>/reports/v180-final-20260512-175855-ats-quality.json`.
- Final source quality JSON: `<app-dir>/reports/v180-postdeploy-20260512-181223-endpoint-ingestion_source-quality.json`.
- Final quarantine summary JSON: `<app-dir>/reports/v180-postdeploy-20260512-181223-endpoint-ingestion_quarantine-summary.json`.
- Final Meili replace report: `<app-dir>/reports/certified-rebuild-20260512-155252-final2-meili-replace-reindex.json`.
- Final Meili post-check JSON: `<app-dir>/reports/v180-postdeploy-20260512-181223-meili-check.json`.
...
- Backup: `<app-dir>/backups/postgres-openjobslots-pre-recruitee-recovery-20260512-203839.dump`.
- Baseline reports: `<app-dir>/reports/recruitee-recovery-before-20260512-203621-*`.
- Write/canary reports: `<app-dir>/reports/recruitee-recovery-write-20260512-203839-*`.
- After/guard reports: `<app-dir>/reports/recruitee-recovery-final2-20260512-204443-*`.
...
- Backup: `<app-dir>/backups/postgres-openjobslots-pre-recruitee-expansion-20260513-085800.dump`.
- Reports: `<app-dir>/reports/recruitee-expansion-20260513-085800-*`.
...
- Backup: `<app-dir>/backups/postgres-openjobslots-pre-applitrack-recovery-20260513-070709.dump`.
- Baseline reports: `<app-dir>/reports/applitrack-recovery-baseline-20260513-064311-*` and `<app-dir>/reports/applitrack-recovery-before-write-20260513-070641-*`.
- Dry-run/canary/apply reports: `<app-dir>/reports/applitrack-recovery-postdeploy-dry-run-20260513-070339.json`, `<app-dir>/reports/applitrack-recovery-canary-apply-20260513-070858.json`, and `<app-dir>/reports/applitrack-recovery-bounded-apply-20260513-071028.json`.
- After/guard reports: `<app-dir>/reports/applitrack-recovery-after-apply-20260513-071241-*`, `<app-dir>/reports/applitrack-recovery-meili-check-20260513-071259.json`, and `<app-dir>/reports/applitrack-recovery-final3-20260513-071850-guard.json`.
...
- Backup: `<app-dir>/backups/postgres-openjobslots-pre-applitrack-expansion-20260513-101744.dump`.
- Report prefix: `<app-dir>/reports/applitrack-expansion-20260513-101744-*`.
- Source recovery report: `<app-dir>/reports/applitrack-expansion-20260513-101744-source-recovery-report.json`.
...
- Backup: `<app-dir>/backups/postgres-openjobslots-pre-zoho-recovery-20260513-104733.dump`.
- Report prefix: `<app-dir>/reports/zoho-recovery-20260513-104733-*`.
- Source recovery report: `<app-dir>/reports/zoho-recovery-20260513-104733-source-recovery-report.json`.
...
- Backup: `<app-dir>/backups/postgres-openjobslots-pre-recruitcrm-recovery-20260513-113654.dump`.
- Report prefix: `<app-dir>/reports/recruitcrm-recovery-20260513-112731-*`.
- Source recovery report: `<app-dir>/reports/recruitcrm-recovery-20260513-112731-source-recovery-report.json`.
...
- Backup: `<app-dir>/backups/postgres-openjobslots-pre-taleo-recovery-20260513-121337.dump`.
- Report prefix: `<app-dir>/reports/taleo-recovery-20260513-121337-*`.
- Source recovery report: `<app-dir>/reports/taleo-recovery-20260513-121337-source-recovery-report.json`.
...
- Backup: `<app-dir>/backups/postgres-openjobslots-pre-public-enabled-growth-20260513-160152.dump`.
- Report prefix: `<app-dir>/reports/public-enabled-growth-20260513-160152-*`.
- Source recovery report: `<app-dir>/reports/public-enabled-growth-20260513-160152-source-recovery-report.json`.
...
- Report prefix: `<app-dir>/reports/cycle-close-20260513-133740-*`.
- Final data quality: `<app-dir>/reports/cycle-close-20260513-133740-data-quality.json`.
- Final ATS quality: `<app-dir>/reports/cycle-close-20260513-133740-ats-quality.json`.
- Final source quality: `<app-dir>/reports/cycle-close-20260513-133740-source-quality.json`.
- Final quarantine summary: `<app-dir>/reports/cycle-close-20260513-133740-quarantine-summary.json`.
- Final Meili check: `<app-dir>/reports/cycle-close-20260513-133740-meili-check-final.json`.
...
- Parser/source readiness is local and fixture-backed until deployment plus bounded live source evidence prove it in production. Do not claim production has the local 62 configured ATS or that disabled/canary sources are recovered before deploy, canary/apply proof, and Meili/Postgres parity pass.
