...
- Current collector target chain: Oracle is the latest local architecture extraction checkpoint and remains `enabled` in the registry to preserve the previous live central-collector posture. Oracle-specific CandidateExperience route parsing, requisitions API pagination, finder/expand metadata, and raw API parsing now live under `server/ingestion/sources/oracle/`; `server/ingestion/sourceCollectors.js`, `server/ingestion/sourceDiscovery.js`, and `server/ingestion/sources/common.js` no longer own Oracle-specific parser/collector logic. Choose the next target from remaining central collector/source-discovery code and configured-company pressure before starting the next batch. No source apply, backfill, cleanup, worker budget change, deploy, or Meili reindex is implied by this target.
