...
- Collector extraction is now the active architecture lane: HRMDirect, BambooHR, ApplyToJob, Breezy, CareerPlug, CareerPuck, GovernmentJobs, SchoolSpring, Simplicant, K12JobSpot, StateJobsNY, CalOpps, HiBob, ApplicantPro, Ashby, Lever, SmartRecruiters, Zoho, RecruitCRM, Recruitee, Fountain, Applitrack, Taleo, PinpointHQ, Join, Rippling, Manatal, Teamtailor, Freshteam, Getro, UltiPro, Workday, ADP MyJobs, ADP WorkForceNow, Greenhouse, iCIMS, isolvisolvedhire, Jobvite, Gem, Paylocity, BrassRing, HireBridge, PageUp, Eightfold, TalentReef, TalentLyft, Oracle, Loxo, CareersPage, CalCareers, and JobAps route through `server/ingestion/sourceRegistry.js` and their own `server/ingestion/sources/<ats>/` modules. `server/ingestion/sourceCollectors.js` should continue shrinking one ATS family at a time.
- Current collector target chain: HiBob is the latest local architecture extraction checkpoint and remains `disabled` with `collectWhenDisabled=false` to preserve its no-live-row posture while enabling guarded certification through the source module. HiBob-specific public board discovery, board-then-`/api/job-ad` fetch metadata, final-host guards, source-id preservation from `jobAdDetails[].id`, `site`/`country` geo evidence, `publishedAt` date preservation, and no-invented-date behavior now live under `server/ingestion/sources/hibob/`; `server/ingestion/sourceCollectors.js` no longer owns HiBob-specific dynamic collector logic. CalOpps, CalCareers, and StateJobsNY remain source-local and disabled/collect-when-disabled off. Read-only next-target recommendation after HiBob is USAJobs first for public-sector source extraction, or TheApplicantManager/ApplicantAI if the next lane prioritizes remaining disabled configured-company pressure. No source apply, backfill, cleanup, worker budget change, deploy, or Meili reindex is implied by this target.
