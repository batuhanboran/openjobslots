...
- Current collector target chain: Greenhouse is the latest local architecture extraction checkpoint and remains `enabled` in the registry to preserve the previous registry posture. Greenhouse-specific board-token discovery, boards API fetch, final-host guards, request metadata, and raw API parsing now live under `server/ingestion/sources/greenhouse/`; `server/ingestion/sourceCollectors.js`, `server/ingestion/sourceDiscovery.js`, and `server/ingestion/sources/common.js` no longer own Greenhouse-specific parser/collector logic. Choose the next target from remaining central collector/source-discovery code and configured-company pressure before starting the next batch. No source apply, backfill, cleanup, worker budget change, deploy, or Meili reindex is implied by this target.
