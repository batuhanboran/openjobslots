...
- Worker throughput defaults are staged at `INGESTION_WORKER_CONCURRENCY=3`, `INGESTION_WORKER_INTERVAL_MS=600000`, `INGESTION_AUTO_SYNC_DAILY_TARGET_BUDGET=12000`, `INGESTION_AUTO_SYNC_TARGETS_PER_RUN=200`, `INGESTION_SOURCE_DAILY_TARGET_BUDGET=1000`, and `INGESTION_ADAPTIVE_SELECTION_LOOKBACK_HOURS=24`. Rollback defaults are documented in `docs/reference/deployment.md`.
- Collector dispatch is registry-driven again: `sourceCollectors.js` now resolves registry aliases through `sourceRegistry.js` instead of carrying a long ATS-specific dispatch chain. The only remaining collector-local legacy paths are `talexio` and `saphrcloud`; SageHR, Peopleforce, and PoliceApp now own fetch/parse behavior in their source modules.
...
- Collector extraction is now the active architecture lane: HRMDirect, BambooHR, ApplyToJob, Breezy, CareerPlug, CareerPuck, GovernmentJobs, SchoolSpring, Simplicant, K12JobSpot, StateJobsNY, CalOpps, HiBob, USAJobs, TheApplicantManager, ApplicantAI, ApplicantPro, Ashby, Lever, SmartRecruiters, Zoho, RecruitCRM, Recruitee, Fountain, Applitrack, Taleo, PinpointHQ, Join, Rippling, Manatal, Teamtailor, Freshteam, Getro, UltiPro, Workday, ADP MyJobs, ADP WorkForceNow, Greenhouse, iCIMS, isolvisolvedhire, Jobvite, Gem, Paylocity, BrassRing, HireBridge, PageUp, Eightfold, TalentReef, TalentLyft, Oracle, Loxo, CareersPage, CalCareers, and JobAps route through `server/ingestion/sourceRegistry.js` and their own `server/ingestion/sources/<ats>/` modules. `server/ingestion/sourceCollectors.js` should continue shrinking one ATS family at a time.
- Current collector target chain: ApplicantAI is the latest local architecture extraction checkpoint and remains `disabled` with `collectWhenDisabled=false` to preserve its disabled/no-expansion posture while enabling guarded certification through the source module. ApplicantAI-specific public careers discovery, HTML fetch metadata, final-host guards, numeric URL-tail source id preservation, location-card evidence, invalid-link skipping, blank-title skipping, and no-invented-date behavior now live under `server/ingestion/sources/applicantai/`; `server/ingestion/sourceCollectors.js` no longer owns ApplicantAI-specific dynamic collector logic. TheApplicantManager, HiBob, USAJobs, CalOpps, CalCareers, and StateJobsNY remain source-local and disabled/collect-when-disabled off. Read-only next-target recommendation after ApplicantAI is SageHR or Peopleforce for remaining vendor-specific non-registry collector gaps. No source apply, backfill, cleanup, worker budget change, deploy, or Meili reindex is implied by this target.
