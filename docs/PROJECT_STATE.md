...
- Collector extraction is now the active architecture lane: HRMDirect, BambooHR, ApplyToJob, Breezy, CareerPlug, ApplicantPro, Ashby, Lever, SmartRecruiters, Zoho, RecruitCRM, Recruitee, Fountain, Applitrack, Taleo, PinpointHQ, Join, Rippling, Manatal, Teamtailor, Freshteam, UltiPro, Workday, ADP MyJobs, ADP WorkForceNow, Greenhouse, iCIMS, isolvisolvedhire, Jobvite, Gem, Paylocity, BrassRing, HireBridge, PageUp, Eightfold, TalentReef, Oracle, and Loxo route through `server/ingestion/sourceRegistry.js` and their own `server/ingestion/sources/<ats>/` modules. `server/ingestion/sourceCollectors.js` should continue shrinking one ATS family at a time.
- Current collector target chain: Loxo is the latest local architecture extraction checkpoint and remains `disabled` in the registry to preserve the previous disabled-source posture while enabling guarded estimates/certification. Loxo-specific `app.loxo.co/<board>` discovery, HTML fetch, final-host guards, request metadata, source-id preservation, and source-local fixtures now live under `server/ingestion/sources/loxo/`; `server/ingestion/sourceCollectors.js` and `server/ingestion/sourceDiscovery.js` no longer own Loxo-specific parser/collector logic. Choose the next target from remaining central collector/source-discovery code and configured-company pressure before starting the next batch. No source apply, backfill, cleanup, worker budget change, deploy, or Meili reindex is implied by this target.
