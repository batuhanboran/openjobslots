...
- Production was read-only checked on `proxmox-lxc100` at `/app` and still runs older code at SHA `72eac3f`; `/health` reports `configured_ats_count=60`, `job_slot_count=347,129`, `visible_ats_count=37`, and `postings_seen_24h_count=35,766`, so the local Personio/Workable modules and Manatal/Gem hardening are not live yet.
