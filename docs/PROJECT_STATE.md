...
- Collector extraction is now the active architecture lane: HRMDirect, BambooHR, ApplyToJob, Breezy, CareerPlug, CareerPuck, GovernmentJobs, SchoolSpring, Simplicant, K12JobSpot, StateJobsNY, ApplicantPro, Ashby, Lever, SmartRecruiters, Zoho, RecruitCRM, Recruitee, Fountain, Applitrack, Taleo, PinpointHQ, Join, Rippling, Manatal, Teamtailor, Freshteam, Getro, UltiPro, Workday, ADP MyJobs, ADP WorkForceNow, Greenhouse, iCIMS, isolvisolvedhire, Jobvite, Gem, Paylocity, BrassRing, HireBridge, PageUp, Eightfold, TalentReef, TalentLyft, Oracle, Loxo, CareersPage, and JobAps route through `server/ingestion/sourceRegistry.js` and their own `server/ingestion/sources/<ats>/` modules. `server/ingestion/sourceCollectors.js` should continue shrinking one ATS family at a time.
- Current collector target chain: CalCareers is the latest local architecture extraction checkpoint and remains `disabled` with `collectWhenDisabled=false` to preserve its no-live-row posture while enabling guarded certification through the source module. CalCareers-specific ASP.NET search endpoint discovery, landing hidden-field extraction, search/row-count/pager postbacks, final-host guards, source-id preservation from `Job Control`/`JobControlId`, Department, Location, Publish Date, public-source fixtures, and invalid-shape behavior now live under `server/ingestion/sources/calcareers/`; `server/ingestion/sourceCollectors.js` no longer owns CalCareers-specific dynamic collector logic. StateJobsNY remains source-local and disabled/collect-when-disabled off. Read-only next-target recommendation after CalCareers is CalOpps first, then USAJobs or HiBob depending on whether the next lane prioritizes public-sector source extraction or configured-company pressure. No source apply, backfill, cleanup, worker budget change, deploy, or Meili reindex is implied by this target.
