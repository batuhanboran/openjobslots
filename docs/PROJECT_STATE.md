...
- `scripts/release-ats-recovery-check.js` now revalidates the preflight proof before release success claims: `ok=true` alone is insufficient if backup file proof, production/expected commit, long-query state, Meili/Postgres delta, worker/autodeploy state, or heavy-job state is missing or unsafe.
