...
- Collector dispatch is registry-driven again: `sourceCollectors.js` now resolves registry aliases through `sourceRegistry.js` instead of carrying a long ATS-specific dispatch chain. The only remaining collector-local legacy paths are `sagehr`, `peopleforce`, `policeapp`, `talexio`, and `saphrcloud`.
