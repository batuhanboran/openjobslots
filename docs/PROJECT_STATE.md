...
- HRMDirect source-quality recovery now includes bounded detail-page enrichment for title-only list rows, using only `table.viewFields` labeled `Location:` / `Department:` evidence. Rows without deterministic detail location evidence remain quarantined; no fake geo/remote inference is allowed.
