...
- Collector extraction is now the active architecture lane: HRMDirect, BambooHR, ApplyToJob, Breezy, CareerPlug, CareerPuck, GovernmentJobs, SchoolSpring, Simplicant, K12JobSpot, StateJobsNY, CalOpps, HiBob, USAJobs, TheApplicantManager, ApplicantPro, Ashby, Lever, SmartRecruiters, Zoho, RecruitCRM, Recruitee, Fountain, Applitrack, Taleo, PinpointHQ, Join, Rippling, Manatal, Teamtailor, Freshteam, Getro, UltiPro, Workday, ADP MyJobs, ADP WorkForceNow, Greenhouse, iCIMS, isolvisolvedhire, Jobvite, Gem, Paylocity, BrassRing, HireBridge, PageUp, Eightfold, TalentReef, TalentLyft, Oracle, Loxo, CareersPage, CalCareers, and JobAps route through `server/ingestion/sourceRegistry.js` and their own `server/ingestion/sources/<ats>/` modules. `server/ingestion/sourceCollectors.js` should continue shrinking one ATS family at a time.
- Current collector target chain: TheApplicantManager is the latest local architecture extraction checkpoint and remains `disabled` with `collectWhenDisabled=false` to preserve its no-live-row posture while enabling guarded certification through the source module. TheApplicantManager-specific public careers discovery, HTML fetch metadata, final-host guards, `pos` query source id preservation, labeled `.pos_location_list` location/remote/hybrid evidence, department preservation, resume-link skipping, and no-invented-date behavior now live under `server/ingestion/sources/theapplicantmanager/`; `server/ingestion/sourceCollectors.js` no longer owns TheApplicantManager-specific dynamic collector logic. HiBob, USAJobs, CalOpps, CalCareers, and StateJobsNY remain source-local and disabled/collect-when-disabled off. Read-only next-target recommendation after TheApplicantManager is ApplicantAI for remaining vendor-specific disabled configured-company pressure. No source apply, backfill, cleanup, worker budget change, deploy, or Meili reindex is implied by this target.
