...
- Collector extraction is now the active architecture lane: HRMDirect, BambooHR, ApplyToJob, Breezy, CareerPlug, CareerPuck, GovernmentJobs, SchoolSpring, Simplicant, ApplicantPro, Ashby, Lever, SmartRecruiters, Zoho, RecruitCRM, Recruitee, Fountain, Applitrack, Taleo, PinpointHQ, Join, Rippling, Manatal, Teamtailor, Freshteam, Getro, UltiPro, Workday, ADP MyJobs, ADP WorkForceNow, Greenhouse, iCIMS, isolvisolvedhire, Jobvite, Gem, Paylocity, BrassRing, HireBridge, PageUp, Eightfold, TalentReef, TalentLyft, Oracle, Loxo, CareersPage, and JobAps route through `server/ingestion/sourceRegistry.js` and their own `server/ingestion/sources/<ats>/` modules. `server/ingestion/sourceCollectors.js` should continue shrinking one ATS family at a time.
- Current collector target chain: Simplicant is the latest local architecture extraction checkpoint and remains `enabled` in the registry to preserve its previous public-enabled dynamic-collector posture while enabling guarded estimates/certification through the source module. Simplicant-specific public HTML board discovery, host blocking, list fetch metadata, final-host guards, source-id preservation from `/jobs/{id}/detail` and `/leads/{id}/detail`, public-source fixtures, and invalid-shape behavior now live under `server/ingestion/sources/simplicant/`; `server/ingestion/sourceCollectors.js` no longer owns Simplicant-specific dynamic collector logic and `server/ingestion/sourceDiscovery.js` no longer owns the Simplicant company parser. Read-only sidecar ranking after Simplicant recommends K12JobSpot next, followed by StateJobsNY and CalCareers. No source apply, backfill, cleanup, worker budget change, deploy, or Meili reindex is implied by this target.
