...
- Collector extraction is now the active architecture lane: HRMDirect, BambooHR, ApplyToJob, Breezy, CareerPlug, ApplicantPro, Ashby, Lever, SmartRecruiters, Zoho, RecruitCRM, Recruitee, Fountain, Applitrack, Taleo, PinpointHQ, Join, Rippling, Manatal, Teamtailor, Freshteam, UltiPro, Workday, ADP MyJobs, isolvisolvedhire, Jobvite, Gem, Paylocity, BrassRing, HireBridge, PageUp, and Eightfold route through `server/ingestion/sourceRegistry.js` and their own `server/ingestion/sources/<ats>/` modules. `server/ingestion/sourceCollectors.js` should continue shrinking one ATS family at a time.
- Current collector target chain: Eightfold is the latest local architecture extraction checkpoint and remains disabled with `collectWhenDisabled=false` until source-backed pagination/safety and live inventory proof are completed. Gem was previously selected as the next target and is already registry-backed; choose the next target from remaining central collector/source-discovery code and configured-company pressure before starting the next batch. No source apply, backfill, cleanup, worker budget change, deploy, or Meili reindex is implied by this target.
