...
- Collector extraction is now the active architecture lane: HRMDirect, BambooHR, ApplyToJob, Breezy, CareerPlug, ApplicantPro, Ashby, Lever, SmartRecruiters, Zoho, RecruitCRM, Recruitee, Fountain, Applitrack, Taleo, PinpointHQ, Join, Rippling, Manatal, Teamtailor, Freshteam, UltiPro, Workday, ADP MyJobs, ADP WorkForceNow, Greenhouse, iCIMS, isolvisolvedhire, Jobvite, Gem, Paylocity, BrassRing, HireBridge, PageUp, Eightfold, TalentReef, TalentLyft, Oracle, Loxo, CareersPage, and JobAps route through `server/ingestion/sourceRegistry.js` and their own `server/ingestion/sources/<ats>/` modules. `server/ingestion/sourceCollectors.js` should continue shrinking one ATS family at a time.
- Current collector target chain: TalentLyft is the latest local architecture extraction checkpoint and remains `disabled` in the registry to preserve a disabled-source posture while enabling guarded estimates/certification. TalentLyft-specific `*.talentlyft.com` discovery, landing-page config extraction, paged `JobList/` fragment fetches, final-host guards, request metadata, `data-job-id` source-id preservation, public-source fixtures, and date-null behavior now live under `server/ingestion/sources/talentlyft/`; `server/ingestion/sourceCollectors.js` and `server/ingestion/sourceDiscovery.js` no longer own TalentLyft-specific parser/collector logic. Choose the next target from remaining central collector/source-discovery code and configured-company pressure before starting the next batch. No source apply, backfill, cleanup, worker budget change, deploy, or Meili reindex is implied by this target.
