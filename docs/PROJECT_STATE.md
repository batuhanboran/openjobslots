...
- Current collector target chain: StateJobsNY is the latest local architecture extraction checkpoint and remains `disabled` with `collectWhenDisabled=false` to preserve its no-live-row posture while enabling guarded certification through the source module. StateJobsNY-specific dated public vacancy table discovery, list fetch metadata, optional bounded detail fetches, final-host guards, source-id preservation from `vacancyDetailsView.cfm?id=...`/Item #, Posted column dates, detail-backed City/State geo, exact `Telecommuting allowed? Yes/No` remote-mode mapping, county-only quarantine behavior, public-source fixtures, and invalid-shape behavior now live under `server/ingestion/sources/statejobsny/`; `server/ingestion/sourceCollectors.js` no longer owns StateJobsNY-specific dynamic collector logic. Read-only sidecar ranking after StateJobsNY recommends CalCareers next, followed by CalOpps. No source apply, backfill, cleanup, worker budget change, deploy, or Meili reindex is implied by this target.
