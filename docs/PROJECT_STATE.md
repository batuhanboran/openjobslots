...
- Current collector target chain: Getro is the latest local architecture extraction checkpoint and remains `disabled` in the registry to preserve a disabled-source posture while enabling guarded estimates/certification. Getro-specific `*.getro.com/jobs` discovery, Next.js `__NEXT_DATA__` list fetch parsing, final-host guards, request metadata, source-id preservation from `id`/`jobId`/`_id`, public-source fixtures, and invalid-shape behavior now live under `server/ingestion/sources/getro/`; `server/ingestion/sourceCollectors.js` and `server/ingestion/sourceDiscovery.js` no longer own Getro-specific parser/collector logic. Choose the next target from remaining central collector/source-discovery code and configured-company pressure before starting the next batch. No source apply, backfill, cleanup, worker budget change, deploy, or Meili reindex is implied by this target.
