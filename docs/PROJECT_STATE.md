...
- Collector extraction is now the active architecture lane: HRMDirect, BambooHR, ApplyToJob, Breezy, CareerPlug, ApplicantPro, Ashby, Lever, SmartRecruiters, Zoho, RecruitCRM, Recruitee, Fountain, Applitrack, Taleo, PinpointHQ, Join, Rippling, Manatal, Teamtailor, Freshteam, UltiPro, Workday, ADP MyJobs, ADP WorkForceNow, Greenhouse, iCIMS, isolvisolvedhire, Jobvite, Gem, Paylocity, BrassRing, HireBridge, PageUp, Eightfold, TalentReef, Oracle, Loxo, CareersPage, and JobAps route through `server/ingestion/sourceRegistry.js` and their own `server/ingestion/sources/<ats>/` modules. `server/ingestion/sourceCollectors.js` should continue shrinking one ATS family at a time.
- Current collector target chain: JobAps is the latest local architecture extraction checkpoint and remains `disabled` in the registry to preserve the previous disabled-source posture while enabling guarded estimates/certification. JobAps-specific `*.jobapscloud.com` discovery, HTML fetch, final-host guards, request metadata, `JobNum` source-id preservation, public-source fixtures, and date-null behavior now live under `server/ingestion/sources/jobaps/`; `server/ingestion/sourceCollectors.js` and `server/ingestion/sourceDiscovery.js` no longer own JobAps-specific parser/collector logic. Choose the next target from remaining central collector/source-discovery code and configured-company pressure before starting the next batch. No source apply, backfill, cleanup, worker budget change, deploy, or Meili reindex is implied by this target.
