...
- Collector extraction is now the active architecture lane: HRMDirect, BambooHR, ApplyToJob, Breezy, CareerPlug, ApplicantPro, Ashby, Lever, SmartRecruiters, Zoho, RecruitCRM, Recruitee, Fountain, Applitrack, Taleo, PinpointHQ, Join, Rippling, Manatal, Teamtailor, Freshteam, UltiPro, Workday, ADP MyJobs, ADP WorkForceNow, isolvisolvedhire, Jobvite, Gem, Paylocity, BrassRing, HireBridge, PageUp, Eightfold, TalentReef, and Oracle route through `server/ingestion/sourceRegistry.js` and their own `server/ingestion/sources/<ats>/` modules. `server/ingestion/sourceCollectors.js` should continue shrinking one ATS family at a time.
- Current collector target chain: ADP WorkForceNow is the latest local architecture extraction checkpoint and remains `enabled` in the registry to preserve the previous live central-collector posture. ADP WorkForceNow-specific `cid`/`ccId` route parsing, content-links company-name evidence fetch, job-requisitions API fetch, final-host guards, request metadata, and raw API parsing now live under `server/ingestion/sources/adp_workforcenow/`; `server/ingestion/sourceCollectors.js`, `server/ingestion/sourceDiscovery.js`, and `server/ingestion/sources/common.js` no longer own ADP WorkForceNow-specific parser/collector logic. Choose the next target from remaining central collector/source-discovery code and configured-company pressure before starting the next batch. No source apply, backfill, cleanup, worker budget change, deploy, or Meili reindex is implied by this target.
