...
- Public health: `331,766` visible job slots, `40,860` companies, `29,767` sync-enabled companies, `37` visible ATS, and `70,088` rows seen in 24h. Source state split is `13` full-enabled, `31` canary, `8` quarantine-only, `10` disabled, and `44` worker-auto-eligible ATS.
- Latest sync status was idle. Worker run `1015` completed with errors: `48/50` target success, `890` postings stored, `0` rejected, queue depth about `24.1k`, and public status parser attention `289`.
- Production data quality: `331,766` visible rows; missing country `21,122` (`6.37%`), missing any normalized geo `44,059` (`13.28%`), weak/unknown remote `9,299` (`2.8%`), and missing-all-geo plus weak/unknown remote `95` (`0.03%`).
- Source freshness posture is `hold`, not scale-up: `30,859` due targets, `3,837` processed in 24h, `90.44%` target success, `12,054` new rows, `0` new public no-geo/no-remote rows, but `266` unresolved parser-attention events plus `266` parser-bug failures, `888` source-quality failures, `79` empty-no-jobs failures, `5` auth failures, `3` rate-limit failures, and `7` unknown failures.
...
- Search parity is not clean. `check-search-parity --api-base-url=https://openjobslots.com --limit=10` failed all 12 default cases in the baseline. Fresh `search:reindex:check -- --json --sample-limit=25` showed document counts aligned (`331,760` Postgres-indexable and `331,760` Meili documents, no missing/extra docs), but returned `ok=false` because remote facets drift by `40` onsite vs unknown documents. Replace reindex or targeted repair still needs explicit approval.
