...
- The source-run summary exposes `planned_batch_required`, `planned_batch_present`, `planned_batch_report_ok`, `planned_batch_report_failures`, `planned_batch_report_selected_tenant_count`, `planned_batch_report_selected_gain`, `planned_batch_predicted_guard_result`, `predicted_guard_result`, and `predicted_guard_ok`, so failed write attempts show whether the missing proof is CLI flags, recovery readiness, unreadable batch proof, source mismatch, or a failed predicted guard.
- Verification covered `node --check server\ingestion\sourceRunner.js` and `node server\ingestion\sourceRunner.test.js` with new negative tests for missing batch proof, unreadable batch report, source mismatch, failed report prediction, and failed CLI prediction. No production source apply, canary/apply, data backfill, public-row delete/hide, Meili repair/reindex, deploy, backup, cleanup, or worker-budget change was run.
