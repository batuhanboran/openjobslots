...
- Collector extraction is now the active architecture lane: HRMDirect, BambooHR, ApplyToJob, Breezy, CareerPlug, ApplicantPro, Ashby, Lever, SmartRecruiters, Zoho, RecruitCRM, Recruitee, Fountain, Applitrack, Taleo, PinpointHQ, Join, Rippling, Manatal, Teamtailor, Freshteam, UltiPro, Workday, ADP MyJobs, isolvisolvedhire, Jobvite, Gem, Paylocity, BrassRing, HireBridge, PageUp, Eightfold, and TalentReef route through `server/ingestion/sourceRegistry.js` and their own `server/ingestion/sources/<ats>/` modules. `server/ingestion/sourceCollectors.js` should continue shrinking one ATS family at a time.
- Current collector target chain: TalentReef is the latest local architecture extraction checkpoint and remains disabled with `collectWhenDisabled=false` until source-backed certification and live inventory proof are completed. TalentReef-specific public route parsing, alias/search API fetches, pagination metadata, and raw search parsing now live under `server/ingestion/sources/talentreef/`; `server/ingestion/sourceCollectors.js`, `server/ingestion/sourceDiscovery.js`, and `server/ingestion/sources/common.js` no longer own TalentReef-specific parser/collector logic. Choose the next target from remaining central collector/source-discovery code and configured-company pressure before starting the next batch. No source apply, backfill, cleanup, worker budget change, deploy, or Meili reindex is implied by this target.
