...
- Collector extraction is now the active architecture lane: HRMDirect, BambooHR, ApplyToJob, Breezy, CareerPlug, CareerPuck, GovernmentJobs, SchoolSpring, Simplicant, K12JobSpot, ApplicantPro, Ashby, Lever, SmartRecruiters, Zoho, RecruitCRM, Recruitee, Fountain, Applitrack, Taleo, PinpointHQ, Join, Rippling, Manatal, Teamtailor, Freshteam, Getro, UltiPro, Workday, ADP MyJobs, ADP WorkForceNow, Greenhouse, iCIMS, isolvisolvedhire, Jobvite, Gem, Paylocity, BrassRing, HireBridge, PageUp, Eightfold, TalentReef, TalentLyft, Oracle, Loxo, CareersPage, and JobAps route through `server/ingestion/sourceRegistry.js` and their own `server/ingestion/sources/<ats>/` modules. `server/ingestion/sourceCollectors.js` should continue shrinking one ATS family at a time.
- Current collector target chain: K12JobSpot is the latest local architecture extraction checkpoint and remains `enabled` in the registry to preserve its previous public dynamic-collector posture while enabling guarded estimates/certification through the source module. K12JobSpot-specific public JSON API discovery, POST search fetch metadata, date-window filtering, final-host guards, source-id preservation from `jobs[].id`, US city/state/postal location evidence, public-source fixtures, and invalid-shape behavior now live under `server/ingestion/sources/k12jobspot/`; `server/ingestion/sourceCollectors.js` no longer owns K12JobSpot-specific dynamic collector logic. Read-only sidecar ranking after K12JobSpot recommends StateJobsNY next, followed by CalCareers. No source apply, backfill, cleanup, worker budget change, deploy, or Meili reindex is implied by this target.
