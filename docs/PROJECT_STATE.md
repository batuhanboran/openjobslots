...
- Collector extraction is now the active architecture lane: HRMDirect, BambooHR, ApplyToJob, Breezy, CareerPlug, CareerPuck, ApplicantPro, Ashby, Lever, SmartRecruiters, Zoho, RecruitCRM, Recruitee, Fountain, Applitrack, Taleo, PinpointHQ, Join, Rippling, Manatal, Teamtailor, Freshteam, Getro, UltiPro, Workday, ADP MyJobs, ADP WorkForceNow, Greenhouse, iCIMS, isolvisolvedhire, Jobvite, Gem, Paylocity, BrassRing, HireBridge, PageUp, Eightfold, TalentReef, TalentLyft, Oracle, Loxo, CareersPage, and JobAps route through `server/ingestion/sourceRegistry.js` and their own `server/ingestion/sources/<ats>/` modules. `server/ingestion/sourceCollectors.js` should continue shrinking one ATS family at a time.
- Current collector target chain: CareerPuck is the latest local architecture extraction checkpoint and remains `enabled` in the registry to preserve its previous public-enabled central-collector posture while enabling guarded estimates/certification through the source module. CareerPuck-specific `app.careerpuck.com/job-board/<board>` discovery, `api.careerpuck.com/v1/public/job-boards/<board>` fetch metadata, final-host guards, request metadata, source-id preservation from `id`/`jobId`/`uuid`, public-source fixtures, and invalid-shape behavior now live under `server/ingestion/sources/careerpuck/`; `server/ingestion/sourceCollectors.js` and `server/ingestion/sourceDiscovery.js` no longer own CareerPuck-specific parser/collector logic. Choose the next target from remaining central collector/source-discovery code and configured-company pressure before starting the next batch. No source apply, backfill, cleanup, worker budget change, deploy, or Meili reindex is implied by this target.
