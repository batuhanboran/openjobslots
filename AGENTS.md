...
- Production host: ltx100 / `public-services`, checkout `/root/OpenPostings`.
...
- After a successful push intended for production, verify ltx100 alignment with the deployment runbook:
  - `git -C /root/OpenPostings rev-parse HEAD`
  - `docker compose --project-directory /root/OpenPostings ps`
  - `curl -fsS http://127.0.0.1:8081/health`
- The ltx100 auto-deploy timer is `openjobslots-deploy.timer`; the deploy log is `/var/log/openjobslots-deploy.log`.
