...
- Hosted runtime: ltx100 / `public-services` at `192.168.1.100`, checkout `/root/OpenPostings`.
- Expected project services on ltx100: `openjobslots-app`, `openjobslots-worker`, `openjobslots-postgres`, `openjobslots-meilisearch`.

## Git And Deployment

- User expects code changes to be committed and pushed to `main` unless they explicitly ask for local-only work.
- The ltx100 deploy key only needs read access. Do not grant write access unless there is a separate server-side push workflow.
- After pushing, verify ltx100 is on the same commit with:
  - `git -C /root/OpenPostings rev-parse HEAD`
  - `docker compose --project-directory /root/OpenPostings ps`
  - `curl -fsS http://127.0.0.1:8081/health`
- The ltx100 auto-deploy timer is `openjobslots-deploy.timer`.
...
- If GitHub SSH deploy key is still not registered, manual alignment can be done with a git bundle over SSH, then rebuilding app and worker:
  - `docker compose --project-directory /root/OpenPostings up -d --build openjobslots-app openjobslots-worker`
...
- Current stabilization checkpoint is `v1.5.17`; the next stabilization update should increment the patch version unless it is a deliberate product rewrite.
...
- Future-use Codex skills installed: `playwright`, `security-best-practices`, `security-threat-model`, `openjobslots-postgres-audit`, and `ats-parser-certification`. A Codex restart may be needed before newly installed skills load.
...
- v1.5.17 adds the ATS certification workbench. Do not mark an ATS as parser-certified unless saved raw fixtures and tests prove geo, date, remote, and source-id behavior, or prove the source omitted a nullable field.
