...
assert.match(deploy, /docker exec -i openjobslots-app node -e/);
assert.match(deploy, /Array\.isArray\(payload\.items\)/);
assert.match(deploy, /Number\.isFinite\(Number\(payload\.count\)\)/);
assert.match(deploy, /validate_postings_response \|\| return 1/);
assert.match(deploy, /openjobslots-web\)" == "healthy" \]\] \|\| return 1/);
