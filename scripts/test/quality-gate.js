...
const preferredTestRoot = process.env.OPENPOSTINGS_QUALITY_GATE_ROOT || "C:\\tmp\\openpostings-quality-gate";
const fallbackTestRoot = path.join(repoRoot, ".tmp", "openpostings-quality-gate");
let testRoot = preferredTestRoot;
let testDbPath = process.env.OPENPOSTINGS_QUALITY_GATE_DB_PATH || path.join(testRoot, "jobs.db");
  ...
  return new Promise((resolve) => {
    ...
    const child = spawn(command, args, {
      ...
      env: {
        ...
        NODE_ENV: "test",
        DB_PATH: testDbPath,
        OPENPOSTINGS_TEST_ROOT: testRoot,
        OPENPOSTINGS_E2E_API_PORT: process.env.OPENPOSTINGS_E2E_API_PORT || "18877",
        OPENPOSTINGS_E2E_WEB_PORT: process.env.OPENPOSTINGS_E2E_WEB_PORT || "19076",
        OPENPOSTINGS_E2E_BASE_URL:
          process.env.OPENPOSTINGS_E2E_BASE_URL ||
          `http://127.0.0.1:${process.env.OPENPOSTINGS_E2E_WEB_PORT || "19076"}`,
        OPENPOSTINGS_API_BASE_URL:
          process.env.OPENPOSTINGS_API_BASE_URL ||
          `http://127.0.0.1:${process.env.OPENPOSTINGS_E2E_API_PORT || "18877"}`,
...
function ensureTestRoot() {
  try {
    ...
    if (process.env.OPENPOSTINGS_QUALITY_GATE_ROOT || process.env.OPENPOSTINGS_QUALITY_GATE_DB_PATH) {
      throw error;
    }
