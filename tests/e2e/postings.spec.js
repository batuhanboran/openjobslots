...
test.describe("postings page QA", () => {
  ...
  test("tapping a mobile source intent chip applies the ATS filter once without query fanout", async ({ page }) => {
    ...
    const input = page.getByTestId("search-input");
    await input.click();
    await input.pressSequentially("greenhouse engineer", { delay: 20 });
    await expect(page.getByTestId("intent-chip-source-greenhouse")).toBeVisible();
    await expectMobileTapTarget(page, "intent-chip-source-greenhouse");

    await page.getByTestId("intent-chip-source-greenhouse").click();
    await expect(page.getByTestId("search-input")).toHaveValue("greenhouse engineer");
    await expect(page.getByTestId("search-suggestions-panel")).toHaveCount(0);
    await page.waitForTimeout(2600);
    ...
    expect(
      calls.postingRequests.filter(
        (request) => request.search === "greenhouse engineer" && request.ats === "greenhouse"
      )
    ).toHaveLength(1);
    expect(calls.postings.filter((search) => search === "greenhouse engineer")).toHaveLength(1);
    await expectNoHorizontalOverflow(page);
  });
  ...
  test("tapping a mobile onsite intent chip applies the non-remote filter once without query fanout", async ({ page }) => {
    const viewport = page.viewportSize() || { width: 1440, height: 900 };
    test.skip(viewport.width >= 768, "mobile onsite intent fanout coverage is covered by the mobile project");
    ...
    const input = page.getByTestId("search-input");
    await input.click();
    await input.pressSequentially("onsite engineer", { delay: 20 });
    await expect(page.getByTestId("intent-chip-onsite")).toBeVisible();
    await expectMobileTapTarget(page, "intent-chip-onsite");

    await page.getByTestId("intent-chip-onsite").click();
    await expect(page.getByTestId("search-input")).toHaveValue("onsite engineer");
    await expect(page.getByTestId("search-suggestions-panel")).toHaveCount(0);
    await page.waitForTimeout(2600);
    ...
    expect(
      calls.postingRequests.filter(
        (request) => request.search === "onsite engineer" && request.remote === "non_remote"
      )
    ).toHaveLength(1);
    expect(calls.postings.filter((search) => search === "onsite engineer")).toHaveLength(1);
    await expectNoHorizontalOverflow(page);
  });
  ...
  test("tapping the mobile theme toggle while typing cancels pending query work", async ({ page }) => {
    const viewport = page.viewportSize() || { width: 1440, height: 900 };
    test.skip(viewport.width >= 768, "mobile utility toggle coverage is covered by the mobile project");
  ...
  test("manually clearing a mobile result query closes an open language menu", async ({ page }) => {
    const viewport = page.viewportSize() || { width: 1440, height: 900 };
    test.skip(viewport.width >= 768, "mobile manual clear overlay coverage is covered by the mobile project");
