...
## Youtube Video
[![OpenJobSlots Discussion](https://img.youtube.com/vi/5sVIhhwx3Yk/0.jpg)](https://www.youtube.com/watch?v=5sVIhhwx3Yk)

## Diagram
![Web UI Screenshot](README-Images/OpenJobSlots_Diagram.png)
...
- https://github.com/batuhanboran/OpenJobSlots/releases/download/v1.0.3/openjobslots-1.0.1-x64.msi

Choose the setup type during install:
- `Typical`: Installs the standard OpenJobSlots app setup (Includes the backend service worker, recommended for most users).
- `Complete`: Installs all available OpenJobSlots features. (Includes the backend service worker and MCP apply agent server, which may not be needed for all users).
...
To turn on the MCP server so your model can interact with OpenJobSlots run:
...
```
[mcp_servers.openjobslots-apply]
command = "node"
args = ['C:\Users\<path to where you cloned the repo>\OpenJobSlots\server\mcp-apply-server.js']
```
